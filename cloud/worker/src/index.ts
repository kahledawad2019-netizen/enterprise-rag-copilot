 ...
 *   2. Identity    - a Cloudflare Access JWT when ACCESS_AUD is configured.
 *                    Verified against the team's JWKS, cached per isolate.
 *                    Skipped entirely when unset, so the stack can be stood up
 *                    before Zero Trust is configured.
...
async function verifyAccessJwt(request: Request, env: Env): Promise<Verdict> {
  ...
  const header = JSON.parse(decodeB64Url(headerB64)) as { kid?: string };
  ...
  const claims = JSON.parse(decodeB64Url(payloadB64)) as {
    aud?: string | string[];
    exp?: number;
    email?: string;
  };
