...
def table(schema: str, name: str, columns: dict[str, str]) -> TableInfo:
    return TableInfo(
        schema=schema, name=name, object_type="BASE TABLE",
...
PAYMENTS = table("billing", "payments", {
    "payment_id": "int",
    "amount": "decimal(19,4)",            # exists HERE, not on invoices
    "tenant_id": "int",
})

CUSTOMER_360 = table("analytics", "vw_customer_360", {
    "customer_id": "int",
...
class TestAggregateTypes:
    ...
    def test_avg_of_a_varchar_is_rejected(self) -> None:
        assert check_aggregate_types(
            "SELECT AVG(status) FROM billing.invoices", context(INVOICES)
        )
    ...
    def test_types_resolve_from_the_catalog_when_the_table_was_not_offered(self) -> None:
        ...
        assert not check_aggregate_types(sql, offered_elsewhere), "expected silence without a catalog"
