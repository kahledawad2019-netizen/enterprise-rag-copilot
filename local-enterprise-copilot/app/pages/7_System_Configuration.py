...
def main() -> None:
    ...
    st.caption("Secrets are never rendered here. The password is a SecretStr and the "
    ...
    st.caption("The real string is built only inside the connection helper and is "
               "never logged.")
    ...
    } for name, profile in PROFILES.items()]), use_container_width=True, hide_index=True)
    ...
    st.caption("`ai` and `security` are absent from allowed_schemas on purpose: the "
    ...
    rows = []
    for package in ("vanna", "qdrant-client", "sqlglot", "streamlit", "pydantic",
                    "sentence-transformers", "ollama", "pyodbc"):
        try:
    ...
    st.caption("vanna reports __version__ = 0.1.0 while the distribution is 2.0.2. "
