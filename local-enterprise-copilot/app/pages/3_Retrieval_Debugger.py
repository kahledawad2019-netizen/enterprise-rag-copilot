...
def main() -> None:
    ...
    query = st.text_input("Query", value="What is the refund policy for enterprise annual plans?")
    compare = st.checkbox(
    ...
    if not st.button("Run retrieval", type="primary"):
        st.info(
            "Suggested queries: `INC-2025-0042` (exact identifier), "
            "`what stops a salesperson cutting the price too far?` (paraphrase)"
        )
        return

    tenant_code, _tenant_id, groups = USERS[user_name]
    context = UserContext(
        user_name=user_name,
        tenant=tenant_code,
    ...
    if compare:
        ...
        for tab, (_name, (results, trace)) in zip(tabs, outcomes.items(), strict=True):
            with tab:
        ...
        st.caption(
            "Shared chunks in the top-k of each pair. Low overlap between "
            "dense and sparse is exactly why fusion helps."
        )
        return

    results, trace = copilot.retriever.retrieve(query, strategy=strategy, user=context, limit=limit)
    ...
    st.caption(
        "Filters run DURING search, not after: filtering afterwards lets "
    ...
    with st.expander("Chunk text"):
        for rank, item in enumerate(results, start=1):
            st.markdown(
                f"**{rank}. [{item.chunk.doc_id} v{item.chunk.version}]** {item.chunk.section_path}"
            )
