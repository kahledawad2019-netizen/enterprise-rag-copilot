...
def show(results, trace, *, verbose: bool) -> None:
    ...
    for rank, item in enumerate(results, start=1):
        ...
        print(f"     meta    : type={chunk.doc_type} authority={chunk.authority} "
              f"access={chunk.access_group} tenant={chunk.tenant}")
        body = " ".join(chunk.text.split())
        width = 400 if verbose else 200
        print(textwrap.fill(body[:width] + ("..." if len(body) > width else ""),
    ...
    try:
        ...
        if args.compare:
            ...
            results, trace = retriever.retrieve(
                query, strategy=args.strategy, user=user, limit=args.limit,
...
def _print_overlap(retriever, query, user, limit) -> None:
    ...
    sets = {
        name: {item.chunk.chunk_id for item in results}
        for name, (results, _) in outcomes.items()
    }
    names = list(sets)
    ...
    for left in names:
        ...
        for right in names:
            ...
            row += f"{shared:>11}"
        print(row)
    print("\n  (cells are the number of shared chunks in the top "
          f"{limit} of each pair)")
